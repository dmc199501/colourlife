...
@interface MCAddCardViewController ()
...
- (void)viewDidLoad {
    ...
    [_userNameTextField setPlaceholder:@"请输入持卡人姓名"];
    ...
    [_cardTextField setPlaceholder:@"请输入银行卡卡号"];
    ...
    UIButton *leaveButton = [[UIButton alloc]initWithFrame:CGRectMake(20, 280, SCREEN_WIDTH-40, 40)];
    [leaveButton setBackgroundColor:BLUK_COLOR];
    
    [leaveButton setTitle:@"申请绑定" forState:UIControlStateNormal];
...
- (void)next{
    ...
    {
        UIAlertView *alertView = [[UIAlertView alloc]initWithTitle:nil message:@"请选择银行卡类型再进行提交" delegate:nil cancelButtonTitle:@"确定" otherButtonTitles:nil];
        [alertView show];
        return;
    }
    if (_userNameTextField.text.length == 0)
    {
        UIAlertView *alertView = [[UIAlertView alloc]initWithTitle:nil message:@"请填写持卡人姓名再进行提交" delegate:nil cancelButtonTitle:@"确定" otherButtonTitles:nil];
        [alertView show];
        return;
    }
    ...
    [MCHttpManager PostWithIPString:BASEURL_AREA urlMethod:@"/hongbao/bindBank" parameters:sendDict success:^(id responseObject)
     {
         ...
         if ([dicDictionary[@"code"] integerValue] == 0 )
         {
            ...
            [SVProgressHUD showSuccessWithStatus:@"申请添加成功"];
             ...
             txVC.name = _userNameTextField.text;
             txVC.bank = typefield.text;
             txVC.card = _cardTextField.text;
             txVC.cardID = dicDictionary[@"content"][@"id"];
