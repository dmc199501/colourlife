//
//  MCZZHomeViewController.m
//  EmployeesEnd
//
//  Created by 邓梦超 on 2017/7/29.
...
- (void)viewDidLoad {
    ...
    [self.buttonMutableArray addObject:@{@"imageName":@"新邮件t",@"title":@"新邮件"}];
    [self.buttonMutableArray addObject:@{@"imageName":@"审批t",@"title":@"审批"}];
    [self.buttonMutableArray addObject:@{@"imageName":@"蜜蜂协同t",@"title":@"蜜蜂协同"}];
    [self.buttonMutableArray addObject:@{@"imageName":@"新e签到t",@"title":@"签到"}];
...
- (void)getHomeButton{
    ...
    for (int i=0; i<officeMutableArray.count; i++) {
        ...
        if ([title isEqualToString:@"新邮件"]||[title isEqualToString:@"审批"]||[title isEqualToString:@"蜜蜂协同"]||[title isEqualToString:@"签到"]) {
...
- (void)setUI{
    ...
    [scanButton addTarget:self action:@selector(scanIng) forControlEvents:UIControlEventTouchUpInside];
