...
#import "MCsalesTableViewCell.h"
@interface MCSalesViewController ()
...
@implementation MCSalesViewController
- (id)init
{
    self = [super init];
    if (self)
...
- (void)loadNewData{
    ...
    NSDictionary *sendDict = @{
                               @"page":@1,
                               @"selType":@(self.classId),
                               @"pagesize":@10,
                               @"appID":@"colourlife",
                               };
    
    [MCHttpManager GETWithIPString:BASEURL_SHOP urlMethod:@"/refund" parameters:sendDict success:^(id responseObject) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath;
{
    ...
    MCsalesTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:indentifier];
    if (cell == nil)
    {
        cell = [[MCsalesTableViewCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:indentifier];
    ...
    [cell.priceLabel setText:[NSString stringWithFormat:@"供货价: %.2f元  建议售价: %.2f元",price,originalPrice]];
