...
@interface MCDGmxViewController ()
...
@implementation MCDGmxViewController
- (id)init
{
    self = [super init];
    if (self)
...
- (void)getdata{
    ...
    [sendDictionary setValue:_dic[@"bno"] forKey:@"uno"];
     [sendDictionary setValue:_dic[@"atid"] forKey:@"atid"];
     [sendDictionary setValue:_dic[@"ano"] forKey:@"ano"];
        ...
        if ([dicDictionary[@"code"] integerValue] == 0 )
        {
            if ([dicDictionary[@"content"] isKindOfClass:[NSDictionary class]])
            {
                ...
                for (int i = 0; i<listMutableArray.count; i++) {
                    if ([[listMutableArray objectAtIndex:i][@"orgcccount"] isEqualToString:[listMutableArray objectAtIndex:i][@"destcccount"] ]) {
                        
                        [listMutableArray removeObjectAtIndex:i];
                    }
                }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath;
{
    ...
    if ([[dataDic objectForKey:@"content"] length] == 0) {
        [cell.bzLabel setText:[NSString stringWithFormat:@"备注:转账"]];
    }else{
    ...
    if ([_dic[@"ano"] isEqualToString:dataDic[@"destcccount"]]) {
        
        if ([[dataDic objectForKey:@"orgclient"] length]>0) {
            [cell.contentLabel setText:[NSString stringWithFormat:@"收到[%@][%@]转入%.2f,交易流水号:%@",[dataDic objectForKey:@"orgplatform"],[dataDic objectForKey:@"orgclient"],[[dataDic objectForKey:@"destmoney"] doubleValue],[dataDic objectForKey:@"tno"]]];
        }else{
        ...
        if ([[dataDic objectForKey:@"destclient"] length]>0) {
            ...
            if ([[dataDic objectForKey:@"destbiz"] length]>0){
             [cell.contentLabel setText:[NSString stringWithFormat:@"向[%@][%@]转出%.2f,交易流水号:%@",[dataDic objectForKey:@"destplatform"],[dataDic objectForKey:@"destbiz"],[[dataDic objectForKey:@"destmoney"]  doubleValue],[dataDic objectForKey:@"tno"]]];
            
            }else{
            
            [cell.contentLabel setText:[NSString stringWithFormat:@"向[%@][%@]转出%.2f,交易流水号:%@",[dataDic objectForKey:@"destplatform"],@"系统",[[dataDic objectForKey:@"destmoney"]  doubleValue],[dataDic objectForKey:@"tno"]]];
